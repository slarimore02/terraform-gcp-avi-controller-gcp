...
| firewall\_se\_data\_rules | The ports allowed for Virtual Services hosted on Services Engines. The configure\_firewall\_se\_data variable must be set to true for this rule to be created | `list(map(string))` | <pre>[<br>  {<br>    "port": "443",<br>    "protocol": "tcp"<br>  },<br>  {<br>    "port": "53",<br>    "protocol": "udp"<br>  }<br>]</pre> | no |
